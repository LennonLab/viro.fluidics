...
2. Visualzing the distribution of outcomes
```{r}
# Load libraries
...
# How much more likely are infected spores to germinate than uninfected? 
library(dplyr)
library(tidyr)

fold_change <- fluid_summary %>%
  group_by(treat, spore.fate) %>%
  summarise(total = sum(count), .groups = "drop") %>%
  pivot_wider(names_from = treat, values_from = total) %>%
  mutate(
    fold_change = Infected / Uninfected,
    percent_change = (Infected - Uninfected) / Uninfected * 100
  )
...
rownames(fate_table) <- c("Uninfected", "Infected")
colnames(fate_table) <- c("Divided", "Lysed", "Stayed")

chisq.test(fate_table)


```
3. Express counts as percent (advice from lab meeting)
```{r}
library(ggplot2)

# Summarize counts and calculate percent within each treatment
fluid_summary.perc <- fluid %>%
  group_by(treat, spore.fate) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(treat) %>%
...
# Rename treatments
fluid_summary.perc <- fluid_summary.perc %>%
  mutate(treat = recode(
    treat,
    "control" = "Uninfected",
    "infect" = "Infected"
...
# Set facet order
fluid_summary.perc$treat <- factor(
  fluid_summary.perc$treat,
  levels = c("Uninfected", "Infected")
)

# Plot percentages
ggplot(fluid_summary.perc,
       aes(x = spore.fate, y = percent, fill = spore.fate)) +
...
4. Looking at how germination time is distributed among infected and uninfected endospores
```{r}
...
5. Test to see if there is a significant difference in germination time (I don't think there is)
```{r}
...
6. Visualize both uninfected and infected germination time plots (like Jay's code)
```{r}
library(ggplot2)
library(dplyr)
...
7. Calculate time to lysis from germination time 
```{r}
